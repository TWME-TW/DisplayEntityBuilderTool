...
public class PlaceData {
	...
	RayTraceResult rayTE = world.rayTraceEntities(eyeLocation, direction, 5, 0.6,
	...
	if (rayTE != null && rayTB != null) {
	    ...
	    if (rayTE.getHitEntity().getLocation().distance(eyeLocation) < rayTB.getHitBlock().getLocation()
		    .distance(eyeLocation)) {
		// player.sendMessage("1: " + rayTE.getHitPosition().getX() + " " +
		// rayTE.getHitPosition().getY() + " " + rayTE.getHitPosition().getZ());
		calculateLocationByEntity(rayTE);
		entityLocation = rayTE.getHitEntity().getLocation();
		placeOnEntity = true;
	    } else {
		// player.sendMessage("2: " + rayTB.getHitPosition().getX() + " " +
		// rayTB.getHitPosition().getY() + " " + rayTB.getHitPosition().getZ());
		calculateLocationByEntity(rayTB);
		placeOnEntity = false;
	    }
	} else if (rayTE != null) {
	    // player.sendMessage("3: " + rayTE.getHitPosition().getX() + " " +
	    // rayTE.getHitPosition().getY() + " " + rayTE.getHitPosition().getZ());
	    calculateLocationByEntity(rayTE);
	    entityLocation = rayTE.getHitEntity().getLocation();
	    placeOnEntity = true;
	} else if (rayTB != null) {
	    // player.sendMessage("4: " + rayTB.getHitPosition().getX() + " " +
	    // rayTB.getHitPosition().getY() + " " + rayTB.getHitPosition().getZ());
	    calculateLocationByEntity(rayTB);
	    placeOnEntity = false;
	}
